...
const MATERIALY = [
  { typ: 'PDF', name: 'instrukcja instalacji.pdf', bytes: PRZYKLADOWY_PDF },
  { typ: 'mp4', name: 'przewodnik wideo.mp4', bytes: PRZYKLADOWY_MP4 },
]
...
test.beforeEach(async () => {
  dataDir = await mkdtemp(path.join(tmpdir(), 'msghub-zal-'))
...
for (const material of MATERIALY) {
  test(`macro z zalacznikiem ${material.typ} kladzie file w schowku Windows jako CF_HDROP`, async () => {
    ...
    await electronApp.evaluate(({ clipboard }) => clipboard.writeText('state-poczatkowy'))
    ...
    // The file lands on the clipboard AFTER the text — a macro inserts content first, then
  // the attachment.
...
test('a file missing from the store does not sink the macro: the text works and the gap is reported', async () => {
  await page.evaluate(() =>
    window.msgHub.saveMacro({
      name: 'Makro z sierota',
      text: '*Tresc dziala mimo braku pliku*',
      attachments: ['att/nie-ma-takiego-pliku.pdf'],
    }),
  )
  ...
  expect(result.missing).toEqual(['att/nie-ma-takiego-pliku.pdf'])
  ...
  expect(onClipboard).toBe('*Tresc dziala mimo braku pliku*')
})
